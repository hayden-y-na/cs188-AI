...
class Graph(object):
    ...
    def __init__(self, variables):
        ...
        """
        "*** YOUR CODE HERE ***"
        # anytime you add into the dictionary, make sure to also append to list
        self.info = dict() # gradient output
        self.nodes = list()
    ...
    def get_inputs(self, node):
        ...
        "*** YOUR CODE HERE ***"
        inputs = []
        for inp in node.get_parents():
            inputs += [self.get_output(inp)]
        # print ("inputs",inputs)
        return inputs
    ...
    def add(self, node):
        ...
        self.info[node] = (output, gradient)
    ...
    def backprop(self):
        ...
        # print self.info[loss_node]

        nodeInfo = self.info[loss_node]
        newNodeInfo = (nodeInfo[0], 1.0)
        self.info[loss_node] = newNodeInfo

        reversedListNodes = reversed(self.nodes)
        for node in reversedListNodes:
            parents = node.get_parents()
            newgradient = node.backward(self.get_inputs(node), self.get_gradient(node))
            ...
            for index in range(length):
                ...
                parentInfo = self.info[parent]
                newParentInfo = (parentInfo[0], parentInfo[1] + newgradient[index])
                self.info[parent] = newParentInfo
...
class Add(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        return [np.ones(inputs[0].shape)*gradient,gradient*np.ones(inputs[1].shape)]
...
class ReLU(FunctionNode):
    ...
    def backward(inputs, gradient):
        "*** YOUR CODE HERE ***"
        x = inputs[0]
        return [np.where(x > 0, gradient, 0)]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        return np.mean(np.array([0.5*(inputs[0] - inputs[1])**2])) 

    @staticmethod
    def backward(inputs, gradient):
        first = ((inputs[0] - inputs[1]) * gradient) / len(inputs[0])
        second = -1 * ((inputs[0] - inputs[1]) * gradient) / len(inputs[0])
        return [first, second]
